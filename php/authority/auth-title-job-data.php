<?php
    ...
    if(isset($_POST['id_find_group']) ){
        ...
        $name=$_POST['name'];
        $stid=oci_parse($conn, "SELECT * FROM CHUCDANH WHERE ID_CD LIKE '".$id."' AND TENCD LIKE '%".$name."%' ORDER BY ID_CD ASC");
        oci_execute($stid);
    ...
    if(isset($_POST['id_job']) && isset($_POST['id_pro'])){
        $id_job=$_POST['id_job'];
        $id_pro=$_POST['id_pro'];
        $stid=oci_parse($conn, "SELECT * FROM PHANQUYEN_NV, CHUCNANG WHERE ID_CT=".$id_pro." 
                                                                        AND ID_CD=".$id_job."
                                                                        AND PHANQUYEN_NV.ID_CN=CHUCNANG.ID_CN
                                                                        ORDER BY PHANQUYEN_NV.ID_CN ASC");
        oci_execute($stid);
        $i=0;
        while($row=oci_fetch_array($stid, OCI_ASSOC +OCI_RETURN_NULLS)){
...
// Tim chuc nang
    if(isset($_POST['id_func_find'])){
        ...
        $stid=oci_parse($conn, "SELECT * FROM PHANQUYEN_NV, CHUCNANG WHERE ID_CT=".$_POST["id_pro_find"]."
                                                        AND ID_CD=".$_POST["id_group_find"]."
                                                        ...
                                                        AND PHANQUYEN_NV.ID_CN=CHUCNANG.ID_CN ORDER BY PHANQUYEN_NV.ID_CN ASC");
        oci_execute($stid);
        $i=0;
        while($row=oci_fetch_array($stid, OCI_ASSOC+OCI_RETURN_NULLS)){
            $i++;
    ...
    if(isset($_POST["sql"])){
        ...
        while($row_cn=oci_fetch_array($stid_cn, OCI_ASSOC+OCI_RETURN_NULLS)){
            $stid_unit=oci_parse($conn, "UPDATE PHANQUYEN_NV SET LOAIDONVI='".$unit."' WHERE ID_CD=".$id_job." AND ID_CN=".$row_cn['ID_CN']);
            oci_execute($stid_unit);
        }
        echo 'Cập nhật phân quyền cho chức danh thành công!';
    }
// Cây chức năng
    ...
    if(isset($_POST['id_func_allow'])&& isset($_POST['id_group_allow']) ){
		...
		$i=0;
		for($CN=0;$CN<$SL;$CN++){
			$stid=oci_parse($conn, "SELECT * FROM PHANQUYEN_NV, CHUCNANG WHERE CHUCNANG.ID_CN=".$id_function[$CN]." 
																			AND ID_CD=".$id_group."
																			AND PHANQUYEN_NV.ID_CN=CHUCNANG.ID_CN
																			ORDER BY PHANQUYEN_NV.ID_CN ASC");
			oci_execute($stid);
...
// Kiem tra loại đơn vị, hiển thị
    if(isset($_POST['id_pro_unit']) && isset($_POST['id_job_unit'])){
        $id_pro=$_POST['id_pro_unit'];
        $id_job=$_POST['id_job_unit'];
        $stid=oci_parse($conn, "SELECT * FROM PHANQUYEN_NV,CHUCNANG WHERE ID_CT=".$id_pro."
                                                    AND ID_CD=".$id_job."
                                                    AND PHANQUYEN_NV.ID_CN=CHUCNANG.ID_CN");
        oci_execute($stid);
