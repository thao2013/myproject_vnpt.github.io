...
function showAuth(){
    var id_job=document.getElementById('id-group-auth').value;
    var id_pro=document.getElementById('id-pro-auth').value;
    var group=document.getElementById('name-group').value;
    var pro=document.getElementById('name-pro').value;
    if(group==''){
        alert('Vui lòng chọn một chức danh để phân quyền!');
    }
    else {
        if(pro==''){
...
function saveAuth(){
	...
	var id_cd=document.getElementById('id-group-auth').value;
    ...
    if(unit==''){
        ...
        for(var i=2;i<rowcount; i++){	
            ...
            query="UPDATE PHANQUYEN_NV SET XEM="+v+", SUA="+e+", XOA="+d+", KHONGXEM="+dv+", KHONGSUA="+de+", KHONGXOA="+dd+" WHERE ID_CD="+id_cd+" AND ID_CN="+id_cn;
            sql.push(query);
        }
        $.ajax({
            type: 'POST',
